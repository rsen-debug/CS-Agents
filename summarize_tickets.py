...
# ── Push CSV to ChurnZero ──────────────────────────────────────────────────
def push_to_churnzero(file_path, url):
    with open(file_path, "rb") as f:
        ...
        try:
            ...
            print(f"File {file_path} successfully posted!")
        except requests.exceptions.RequestException as e:
            print(f"An error occurred posting {file_path}: {e}")
            raise
...
def main():
    ...
    # Extract AccountExternalId and CzTicketIdentifier
    temp_raw_tickets = raw_tickets_df[[id_col, "Account", "CzTicketIdentifier"]].copy()
    temp_raw_tickets["AccountExternalId"] = temp_raw_tickets["Account"].apply(
        lambda x: x.get("ExternalId") if isinstance(x, dict) else None
    )
    temp_raw_tickets[id_col] = temp_raw_tickets[id_col].astype(str)

    df_merged = pd.merge(
        df_summaries,
        temp_raw_tickets[[id_col, "AccountExternalId", "CzTicketIdentifier"]],
        left_on="Ticket ID",
        right_on=id_col,
        how="left",
    )

    # Aggregate summaries by account
    df = (
    ...
    df_ZD_tickets = df_merged.drop(columns=["Ticket ID"])
    df_ZD_tickets.to_csv("df_ZD_tickets.csv", index=False)
    print("Saved df_ZD_tickets.csv")

    # ── Push to ChurnZero accounts table ──────────────────────────────────
    push_to_churnzero(
        "sentiment_results.csv",
        f"{CZ_BASE_URL}/batchAccountsCsv?appKey={CZ_APP_KEY}"
    )

    # ── Push to ChurnZero custom tickets table ────────────────────────────
    push_to_churnzero(
        "df_ZD_tickets.csv",
        f"{CZ_BASE_URL}/batchCustomTableCsv?customTableId=7&appKey={CZ_APP_KEY}"
    )
